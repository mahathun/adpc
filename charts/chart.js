...
//             });
   ...
   // Add the Y Axis - min temp
    ...
    drawCharts(dataset,label);
}
function loadQueryB(qb,label){

     d3.select("svgtmin").remove();


     svgtmin = d3.select("#tmin")
    .append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
    .append("g")
   ...
   // var axrf = svgrf.selectAll(".axis");
    ...
    var lbl2 = svgtmin.selectAll(".label");
...
//drawing the chart
function drawCharts(data,label){
    dataCurrent = data;
...
//drawing the chart 2
function drawCharts2(data,label){
    dataCurrent = data;
